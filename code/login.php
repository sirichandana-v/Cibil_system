	...
	$password = sanitize(trim($_POST['password']));
			...
			$sql_stud = "SELECT * from students where username='$username' and password = '$password'";
				...
				if($row['username'] == $username && $row['password'] == $password){
					...
					$sql_banks = "SELECT * from banks where username='$username' and password = '$password'";
				$query = mysqli_query($conn, $sql_banks);
				$row = mysqli_fetch_assoc($query);
				if($row['username'] == $username && $row['password'] == $password){
					$_SESSION['bank-username'] = $row['username'];
					$_SESSION['bank-name'] = $row['name'];
					$_SESSION['bank-matric'] = $row['matric_no'];
						header("Location:bankportal1.php");
					}
					else {
						echo"<div class='alert alert-danger alert-dismissable'>
						<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
			...
			<div class="container">
				<form class="form-horizontal" role="form" method="post" action="login.php" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<br><br>New<a href="addstudent.php"> USER</a>/<a href="addbank.php"> BANK</a>?Register here!!</a>
						</div>
					</div>
